// 12 flat-illustrated animal faces, composed from simple shapes. Each is a
// component returning a 100x100 SVG that sits on a tinted circular swatch.
// Ported from the locked design (prototype/launcher/avatars.jsx).
//
// The API reserves avatar_id 0-19; only 0-11 are designed. Unknown ids fall
// back to the Fox so the UI never breaks. See README "Open / undecided".
...
export const AVATAR_BY_ID: Record<number, AvatarEntry> = Object.fromEntries(
  AVATAR_LIST.map((a) => [a.id, a]),
);

export function avatarFor(id: number): AvatarEntry {
  return AVATAR_BY_ID[id] ?? AVATAR_LIST[0];
}

/** Circular avatar swatch. `ring` adds the paper halo used in the add-profile preview. */
export function Avatar({ id, size = 124, ring = false }: { id: number; size?: number; ring?: boolean }) {
